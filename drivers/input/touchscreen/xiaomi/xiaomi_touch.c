...
int knock_node_init(void) {
	...
	if (knock_data.vaddr == NULL) {
		...
		if (knock_data.vaddr != 0) {
			knock_data.paddr = (void*)virt_to_phys(knock_data.vaddr);
...
int knock_data_mmap(struct file* file, struct vm_area_struct *vma) {
	...
	if (0 == remap_pfn_range(vma, vma->vm_start, pfn, size, __pgprot(0x68000000000fc3))) {
		pr_info("%s: remap_pfn_range %u, size:%ld, success\n", __func__, (unsigned int)pfn, size);
	} else {
		return -EAGAIN;
	}
	return 0;
...
ssize_t knock_data_read (struct file * dev, char __user * buf, size_t count, loff_t * pos)
{
	char local_buf [5] = {0};
	unsigned long not_copied = 0;
	if(*pos == 0) {
		int size = snprintf(local_buf,5,"%d\n", knock_data.frame_count);
		if (size < 0) {
			return size;
		}
...
static int xiaomi_touch_dev_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if(remap_pfn_range(vma, start, pfn, size, __pgprot(0x68000000000fc3))) {
		return -EAGAIN;
	} else {
		pr_info( "%s remap_pfn_range %u, size:%ld, success\n", __func__, (unsigned int)pfn, size);
	}
...
int copy_touch_rawdata(char *raw_base,  int len)
{
	...
	if (touch_pdata->raw_tail != RAW_BUF_NUM - 1)
	...
	if (sizeof(int) * para_cnt < MAX_BUF_SIZE) {
		...
		if ((touch_pdata->param_head != touch_pdata->param_tail)
			...
			if (touch_data->touch_event_status != 0) {
				...
				res = prepare_to_wait_event(&touch_data->wait_queue, &wait_entry, 1);
				if (touch_data->touch_event_status == 0) {
					j = 0x19;
				} else {
					j = 0x19;
					do {
						...
						res = prepare_to_wait_event(&touch_data->wait_queue, &wait_entry, 1);
						if (touch_data->touch_event_status == 0 && j == 0) {
							j = 1;
						}
					} while ((j != 0) && (touch_data->touch_event_status != 0));
				}
				res = j;
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%d\n", touch_pdata->touch_data[0]->irq_no);
}
...

{
	...
	if (pdata->abnormal_event_head == pdata->abnormal_event_tail
		...
		memcpy(buf, pdata->abnormal_event_buf[pdata->abnormal_event_head], ABNORMAL_EVENT_SIZE);
					 ...
					 struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!pdata || count != ABNORMAL_EVENT_SIZE) {
		pr_err("[MITouch-ERR][%s:%d] %s fail! pdata = %ld, size = %d, %d\n", __func__, __LINE__, __func__, (long)pdata, count, ABNORMAL_EVENT_SIZE);
		return -ENODEV;
	}
	...
	memcpy(pdata->abnormal_event_buf[pdata->abnormal_event_tail], buf, ABNORMAL_EVENT_SIZE);
...
static int xiaomi_touch_probe(struct platform_device *pdev)
{
	...
	pdata->raw_data = (unsigned int *)kzalloc(RAW_SIZE, GFP_KERNEL); // flag: 0xdc0
	if (!pdata->raw_data) {
		ret = -ENOMEM;
	...
	for (i = 0; i < PARAM_BUF_NUM; i++) {
		pdata->touch_cmd_data[i] = kzalloc(sizeof(struct touch_cmd_info), GFP_KERNEL);
