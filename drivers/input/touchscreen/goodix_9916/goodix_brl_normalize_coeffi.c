...
int goodix_normalize_coeffi_update(goodix_ts_core *cd) {
    ...
    if (fw->size < 100) {
        ts_err("request_firmware, normalize param length error,len:%zu", fw->size);
        return -EINVAL;
    }
    ...
    if (cd->ic_info.parm.mutual_freq_num != 0) {
        for (i = 0; i < cd->ic_info.parm.mutual_freq_num; i++) {
            ...
            normalize_k_array[i] = (const struct normalize_k_param){ 0 };
...
other_err:
            ...
            normalize_k_array[i].size = normalize_k_array[i].head.tx_num * normalize_k_array[i].head.rx_num;
            normalize_k_array[i].data = kmalloc(normalize_k_array[i].size * sizeof(u16) , GFP_KERNEL);
            ...
            if (normalize_k_array[i].size > 0) {
                for (j = 0; j < normalize_k_array[i].head.tx_num; j++) {
                    for (k = 0; k < normalize_k_array[i].head.rx_num; k++) {
                        token = strsep(&ptr, ",");
                        if (token == NULL || kstrtoint(token, 10, &tmp) != 0) {
                            rc = -4;
                        ...
                        normalize_k_array[i].data[j * normalize_k_array[i].head.rx_num + k] = tmp;
                    }
                    goto_next_line(&ptr);
                    if (ptr == NULL || *ptr == '\0') {
                        rc = -5;
    ...
    if (rc < 0) {
    ...
    } else if ((normalize_k_array[0].size == 0) || (normalize_k_array[0].data == NULL)) {
        ...
        if (cd->ic_info.other.normalize_k_version == 0xffff) {
            ...
            ts_info("K version unequal need update 0x%x != 0x%x", normalize_k_array[0].head.version, cd->ic_info.other.normalize_k_version);
        }
